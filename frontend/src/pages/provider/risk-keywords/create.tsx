// v1.0.0 — 服务商风控关键词新增页(对齐 admin/risk-keywords/create.tsx)。
...
// category → 默认 level 映射(L1 / L2)
const DEFAULT_LEVEL: Record<string, string> = {
  owner_abuse: "L1",
  owner_threat: "L2",
  agent_violation: "L2",
  agent_minor_misconduct: "L1",
};

export function ProviderRiskKeywordCreatePage() {
  const go = useGo();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createKw(
      { resource: "provider/risk-keywords", values: form },
      {
        onSuccess: () => go({ to: "/provider/risk-keywords" }),
        onError: (err) => {
  ...
  return (
    <div className="max-w-lg">
      <div className="flex items-center gap-3 mb-6">
        <button
          type="button"
          onClick={() => go({ to: "/provider/risk-keywords" })}
      ...
      >
        <div>
          ...
          <input
            ...
            style={{ borderColor: "var(--color-neutral-300)", borderRadius: "var(--radius-md)" }}
            placeholder="如:投诉、威胁..."
          />
        </div>
          ...
          <select
            ...
            onChange={(e) =>
              setForm({
                ...
                level: DEFAULT_LEVEL[e.target.value] ?? "L1",
              })
            }
            className="w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2"
            style={{ borderColor: "var(--color-neutral-300)", borderRadius: "var(--radius-md)" }}
