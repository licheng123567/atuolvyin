...
export function RiskKeywordEditPage() {
  ...
  const go = useGo();
  const { mutate: updateKw, mutation: updateMutation } = useUpdate();
  ...
  if (isLoading) {
    return (
      <div className="py-10 text-center text-[var(--color-neutral-400)]">加载中...</div>
    );
  }
      ...
      >
        ...
        <div>
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-[var(--color-neutral-700)]">启用此关键词</span>
          </label>
        </div>
