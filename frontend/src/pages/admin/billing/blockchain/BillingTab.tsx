// v0.8.0 Wave C — 「存证管理」双 tab 中的「计费视图」(原 index.tsx 内容)
//
// 数据源:
...
//
// 月份选择上移到 index.tsx 父容器,本组件接 yearMonth prop。
import { useCustom } from "@refinedev/core";
import { ExternalLink, Hash, Link2 } from "lucide-react";
...
export function BillingTab({ yearMonth }: BillingTabProps) {
  ...
  const { query: listQ } = useCustom<PaginatedResponse<AttestationItem>>({
    ...
    method: "get",
    config: { query: { year_month: yearMonth, page, page_size: 30 } },
  });
