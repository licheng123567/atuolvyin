...
def upload_recording(
    ...
    dev = db.execute(
        text("SELECT id, agent_phone FROM device WHERE device_id=:d"), {"d": device_id}
    ).fetchone()
    if not dev:
...
def submit_business(call_id: int, payload: dict, db: Session = Depends(get_db)):
    ...
    task_id = cl[0]
    t = (
        db.execute(text("SELECT type, owner_id, payload FROM task WHERE id=:t"), {"t": task_id})
        .mappings()
        .fetchone()
    )
    if not t:
    ...
    if t["type"] == "vote":
        ...
        """),
            dict(
                o=t["owner_id"],
                m=t["payload"]["motion_id"],
                c=payload.get("choice", "未明确"),
                cid=call_id,
                n=payload.get("note"),
            ),
        )
