...
def _resolve_law_firm_for_user(db: Session, user_id: int, role_in_firm: str = "admin") -> LawFirmMembership:
    stmt = (
...
async def assign_lawyer_to_order(
    ...
    if order.status != "dispatched":
        raise HTTPException(
            ...
            detail={"code": "ERR_INVALID_STATE", "message": f"订单当前状态为 {order.status}，仅 dispatched 可分律师"},
        )
