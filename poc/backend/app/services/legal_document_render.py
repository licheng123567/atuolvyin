...
def build_order_render_context(
    ...
    if case is not None:
        ctx["amount_owed"] = (
            f"{float(case.amount_owed):,.2f}" if case.amount_owed else "0.00"
        )
        ...
        if owner is not None:
            ...
            ctx["owner_address"] = (
                f"{building} {room}".strip() if (building or room) else "[未填]"
            )
...
def get_template_for_order(
    ...
    tenant_tpl = db.execute(
        select(LegalDocumentTemplate).where(
        ...
        ).order_by(LegalDocumentTemplate.id.desc()).limit(1)
    ).scalar_one_or_none()
...
def render_for_order(
    db: Session, *, order: LegalConversionOrder, rendered_by: int | None = None
):
