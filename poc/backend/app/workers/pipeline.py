...
def process_call(call_log_id: int):
    ...
    try:
        ctx = db.execute(text("""
            ...
            WHERE cl.id=:c
        """), {"c": call_log_id}).mappings().fetchone()
        if not ctx:
            logger.error("call %s not found", call_log_id)
            return

        # 1. ASR — 优先用本地文件路径（避免依赖公网 URL）
        local_path = f"{settings.local_storage_root}/{ctx['object_key']}" \
            if settings.storage_backend == "local" else None
        try:
        ...
        db.execute(text("UPDATE call_log SET status='transcribed' WHERE id=:c"),
                   {"c": call_log_id})
        db.commit()
        ...
        """), dict(c=call_log_id, t=ctx["task_type"],
                   f=json.dumps(extraction["fields"], ensure_ascii=False),
                   conf=extraction.get("confidence"),
                   m=extraction["model"], nr=extraction.get("needs_review", False)))
        db.execute(text("UPDATE call_log SET status='extracted' WHERE id=:c"),
                   {"c": call_log_id})
        db.commit()
