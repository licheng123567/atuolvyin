...
>
> **P1 标记口径调整（2026-05-07，v1.4 S16.6）**：原 P1 = "未上线"；现 P1 = "已上线但数据/UX 待打磨" — 即页面已实现并接通真实接口，但 demo 数据稀疏 / 视觉精细度未达 P0 标准。下一版本（v1.5）会补 demo 数据并打磨 UX。
> 例：admin/reports（数据报表）、admin/compliance（合规月报）保留 P1；admin/providers（服务商合作）已具备完整功能（含 v1.4 推荐入驻），从 P1 摘除。
