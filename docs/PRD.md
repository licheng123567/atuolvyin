...
```
ASR 文本 → 关键词引擎（Aho-Corasick，<50ms）
...
| 级别 | 触发 | App | PC | 后端 |
|------|------|-----|----|----|
| L1 提示 | 低风险词 | 震动 + 黄色 Toast | 右栏黄字建议 | 记录，不通知 |
| L2 警告 | LLM risk≥2 | 强震动 + 全屏警告 | 橙色弹窗 + 督导通知 | 督导实时告警 |
| L3 强制挂断 | LLM risk=3 或 L2 被忽略 60s | **call.disconnect()** | 自动标注留证 | 完整风控日志 |

**L3 开关策略**：L3 自动挂断**默认关闭**，需管理员在「系统配置 → 风控设置」中手动启用。
开关存储在租户级配置表，不影响 L1/L2。未启用时 L3 触发条件满足后降级为 L2 警告 + 督导强通知。

### 12.3 关键词分类

- **业主辱骂**（→L1）：人身攻击类，支持模糊匹配和变体
- **业主威胁**（→L2）：投诉/媒体/律师/12345/上法院/曝光
- **员工违规**（→L2）：承诺减免费用/随便你/爱交不交
...
| 2.3 | **租户详情** | 基本信息（公司名/信用代码/管理员手机）；订阅套餐与有效期；当前用量（用户数/案件数/本月通话分钟）；签约服务商；跟进记录时间线 | P0 |
...
| # | 页面 | 核心功能 | 优先级 |
|---|------|---------|--------|
| 2.1 | **服务商概览** | 签约物业公司数、本月完成通话数、本月应收金额、团队成员数；各客户待处理事项入口 | P0 |
...
| # | 页面 | 核心功能 | 优先级 |
|---|------|---------|--------|
| 3.1 | **管理看板首页** | 今日外呼量/接通量/承诺缴费数/回款金额；全员排名；公海案件数；AI 话术采用率趋势；风控告警摘要 | P0 |
...
| # | 页面 | 核心功能 | 优先级 |
|---|------|---------|--------|
| 4.1 | **督导工作台首页** | 待复核通话数；本组今日绩效摘要；升级至我的案件数；风控告警（本组）| P0 |
...
| # | 页面 | 核心功能 | 优先级 |
|---|------|---------|--------|
| 5A.1 | **今日任务列表（首页）** | 私海案件列表，按优先级排序（欠费金额/逾期月数）；状态筛选（待联系/跟进中/承诺中）；今日完成数角标 | P0 |
...
| 5B.4 | **个人绩效** | 本月通话量/转化率/排名位次（全公司排名，只显示自己的位置，不显示他人数据）| P1 |
...
**MVP P0 页面约 50 个**，P1 页面（约 32 个）在 v1.1 交付。其中角色 3/4 新增 +1 页为「项目列表」管理页（内嵌于各自后台），角色 10/11 共用 `project-manager.html` 4 页。
